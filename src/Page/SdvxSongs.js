...
import { createSdvxSongFilterConfig } from '../config/songFilterConfigs';
      ...
      try {
        ...
        const processedSongs = data.songs.map((song) => ({
          ...
          sheets: (song.sheets || []).map((sheet) => {
            const parsedLevel = parseFloat(sheet.level);
            return {
              ...sheet,
              difficulty: sheet.difficulty || sheet.difficultyAbbr || 'UNKNOWN',
              levelValue: sheet.levelValue ?? (Number.isNaN(parsedLevel) ? 0 : parsedLevel),
              internalLevelValue:
                sheet.internalLevelValue ?? sheet.levelValue ?? (Number.isNaN(parsedLevel) ? 0 : parsedLevel),
  ...
  const columns = useMemo(
    () => [
        ...
        render: (sheets = []) => (
          <Space wrap>
            {sheets.map((sheet, index) => (
              <Tag key={`${sheet.difficulty}-${index}`} color={getDifficultyColor(sheet.difficulty)}>
      ...
      },
      {
        ...
        dataIndex: 'type',
        key: 'type',
        render: (type) => <Tag color={getTypeColor(type)}>{type || '未知类型'}</Tag>,
      },
    ],
    []
  );
  ...
  return (
    <ResponsiveLayout>
      <div className="song-page-shell">
        <Title level={2}>SDVX 曲库</Title>
        <Paragraph>SOUND VOLTEX 曲库数据，包括歌曲的难度、版本和类型信息。</Paragraph>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: 50 }}>
            <Spin size="large" />
            <div style={{ marginTop: 20 }}>正在加载 SDVX 曲库数据...</div>
          </div>
        ) : (
          <>
            <SongFilterModal
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16, marginTop: 12 }}>
                    {selectedRandomSongs.map((song) => (
                      <Card
                        ...
                        extra={<Button type="text" size="small" icon={<CloseOutlined />} onClick={(event) => { event.stopPropagation(); removeRandomSong(song.songId); }} />}
                      >
                        <Space align="start">
              ...
              <Row gutter={[12, 12]}>
                {(selectedSong.sheets || []).map((sheet, index) => (
                  <Col xs={24} sm={12} key={`${sheet.difficulty}-${index}`}>
                    <Card size="small">
                      <Tag color={getDifficultyColor(sheet.difficulty)}>{sheet.difficulty}</Tag>
